...
/*	function __construct()
	...
	public static function getInstance()
	{
		if (is_null(self::$instance))
			self::$instance = new debButtonsAdmin();
	...
	*/
	...
	public function getButtons($args = array())
	{
		...
		$limit = apply_filters('debbuttons/list/num_items', $limit);
...
//		echo '<div class="tablenav top"> ';
		echo "<span class='hint'>" . __('Click on a button to select it and add the shortcode to the editor', 'debbuttons') . "</span>";
...
//		echo '</div>';
		...
		if (count($buttons) == 0)
		{
 			...
 			$url = admin_url('admin.php?page=debbuttons-controller&action=edit');
			echo "<h3 class='nobuttons-header'>" . __("You didn't create any buttons yet!","debbuttons") . "</h3>";
			echo "<p class='nobuttons-link'>" . sprintf(__("Click %shere%s to add one", "debbuttons"),
	...
	static public function mediaShortcodeOptions($post)
	{
		...
		$preview->content = '<h3>' . __('Shortcode Options') . '</h3>
												...
												<p>' . __('Change the options to add shortcode attributes. ', 'debbuttons') . '</p>';
		...
		$url->label = __('Button URL', 'debbuttons');
		$url->value = $data['url'];
		...
		$text->name = $text->id;
		$text->label = __('Button Text', 'debbuttons');
		...
		$more->name = 'more';
		$more->content = '<div class="more-options"><a href="#">' . __('More Options', 'debbuttons') . '</a></div>';
		...
		$new_window->label = __('Open in New Window', 'debbuttons');
		$new_window->value = 1;
		...
		$ffollow->label = __('Use rel="nofollow"', 'debbuttons');
		$ffollow->value = 1;
		...
		$download->label = __('URL is download', 'debbuttons');
	...
	function log($action, $message)
	{
		...
		$file = fopen( trailingslashit($dir) . "/debbuttons.log", "a+");
		$now = new \DateTime();
