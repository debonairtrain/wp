...
class debButton
{
	...
	*/
	function set($id = 0, $name = '', $status = 'publish')
	{
		...
		/* Take the id from the query, otherwise ( when button shortcode called by name ) id might not be present properly' */
 		$button_id = $row["id"];
 		debButtons::buttonLoad(array("button_id" => $button_id)); // central registration
	...
	*/
	...
	function setupData($data)
	{
		...
		// Because PHP 5.3
		$class = maxUtils::namespaceit('debButtons');
...
//echo "<PRE>PARSE " . $this->id . " ;; "; print_R($css); echo "</PRE>";
	...
	*/
	function parse_button($mode = 'normal')
	{
		...
		$classes = array("debbutton-" . $this->id,
						 "debbutton");


		if ($name != '')
			$classes[] = "debbutton-" . $name;
	...
	public function display($args = array() )
	{
		...
		if ($preview)  // mark it preview
		{
			...
			$domObj->find('a',0)->class .= ' debbutton-preview';
		}
		...
		if ($preview && $args["preview_part"] != 'full')
		{
 	 		...
 	 		if ($args["preview_part"] != 'normal')
 	 		{
				...
				$domObj->find('a', 0)->id = 'debbuttons_preview_hover';
			...
			else
			{
				...
				$domObj->find('a', 0)->id = 'debbuttons_preview_normal';
		...
		debButtons::buttonDone(array("button_id" => $this->id, "document_id" => $this->document_id) );
	...
	public function display_js($echo = true, $tag = true)
	{
		...
		if ($tag)
		{
			...
			$output .= " if (typeof debbutton" . $this->document_id . " == 'undefined') { ";
			$output .= " function debbutton" . $this->document_id . "() { ";
		}
		...
		if ($tag)
		{
			$output .= " }
				...
				window.onload = debButton" . $this->document_id . "();
				</script>		";
		}
