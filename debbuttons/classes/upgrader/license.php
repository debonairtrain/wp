...
class upgradeLicense
{
	...
	protected $product_id = "debbuttons-pro";
	protected $license_key = null;
	protected $license_activated = null;
	protected $license_lastcheck = null;
	protected $license_expires = null;

	protected $api_url = 'https://www.debonairtraining.com';
//	protected $update_url = 'https://debonairtraining.com/maxupdate';
	...
	public function activate_license($license_key)
	{
		...
		if (isset($data->license) && $data->license == 'valid')
		{
			...
			update_option('debbuttons_pro_license_key', $license_key, true);
			update_option('debbuttons_pro_license_expires', $expires );
			update_option('debbuttons_pro_license_activated', true, true);
	...
	protected function do_api_post($args)
	{
			...
			if ( is_wp_error( $result ) || 200 !== wp_remote_retrieve_response_code( $result ) )
			{
				...
				$message =  ( is_wp_error( $result ) && ! empty( $error ) ) ? $error : __( 'An connection error occurred, please try again.', 'debbuttons-pro');
	...
	protected function handle_error($data)
	{
			...
			switch( $data->error ) {
					case 'expired' :
						$message = sprintf(
							__( 'Your license key expired on %s.', 'debbuttons-pro' ),
							date_i18n( get_option( 'date_format' ), strtotime( $data->expires, current_time( 'timestamp' ) ) )
						);
						break;
					case 'revoked' :
					case 'disabled' :
						$message = __( 'Your license key has been disabled.','debbuttons-pro');
						break;
					case 'missing' :
						$message = __( 'Invalid license. Please check the license code. ', 'debbuttons-pro');
						break;
					case 'invalid' :
					case 'site_inactive' :
						$message = __( 'Your license is not active for this URL.', 'debbuttons-pro' );
						break;
					case 'item_name_mismatch' :
						$message = sprintf( __( 'This appears to be an invalid license key for %s.', 'debbuttons-pro' ), 'DebButtons PRO' );
						break;
					case 'no_activations_left':
						$message = __( 'Your license key has reached its activation limit.','debbuttons-pro' );
						break;
					default :
						$message = __( 'An error occurred, please try again.', 'debbuttons-pro' );
						break;
			}
	...
	public function get_remote_license($license_key)
	{
		...
		//	$this->update_license_checked_time( (3*HOUR_IN_SECONDS) );
			...
			return __("HTTP Request failed", 'debbuttons');
		}
