...
class maxInstall
{
	...
	public static function activate_plugin($gocreate = true)
	{
		...
		update_option(DEBBUTTONS_VERSION_KEY, DEBBUTTONS_VERSION_NUM);
	...
	*/
	static function migrate()
	{
		...
		$table = maxUtils::get_table_name();
 		...
 		if (! self::debbuttons_database_table_exists($old_table))
 		{
 			return;
 		}
...
//										 echo " <!- DOING $fname -> <BR> ";
	...
	{
			delete_option(DEBBUTTONS_VERSION_KEY);
	}


	static function debbuttons_database_table_exists($table_name) {
		global $wpdb;
	 ...
	 //global $debbuttons_installed_version;
