...
use DebButtons\Upgrader\upgradeLicense as upgradeLicense;
use DebButtons\Upgrader\proInstaller as proInstaller;

class upgradeController extends MaxController
{

  protected $view_template = 'debbuttons-pro';
  ...
  public function handlePost()
  {
		if (check_admin_referer('upgrade', 'upgrade_nonce'))
		{
			...
			//var_dump(wp_redirect(admin_url('admin.php?page=debbuttons-license&installedfrom=debbuttons-free')));
			//exit();
				...
				if (strlen($licenseKey) == 0 )
					$this->view->PostError = __('Enter a valid license key to proceed', 'debbuttons');
				else
	...
	private function checkLicense($licenseKey)
	{
			...
			elseif ($result->success)
			{
				 	if ($result->license == 'valid')
					{
							 ...
							 {
							 	 $this->view->PostError =  __('License seems active, but something went wrong acquiring the download', 'debbuttons');
							 }
	...
	private function installPlugin($url)
	{
			...
			if ($result['success'])
			{
				 ...
				 if ($result['success'])
				 {
					  $this->view->PostError = __('Everything seems fine, plugin should have redirected to the PRO version', 'debbuttons');
				 }
				 else
				 {
	...
	private function updateLicense()
	{
			...
			update_option('debbuttons_pro_license_activated', false, true);
	}
