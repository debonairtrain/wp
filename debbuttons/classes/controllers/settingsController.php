...
class settingsController extends MaxController
{
  ...
  protected $view_template = 'debbuttons-settings';
  ...
  {
		 if (! check_admin_referer('action-settings-form', 'debbuttons-settings-nonce'))
		 {
			 exit('Invalid Nonce');
		 }
    ...
    if (isset($_POST["reset_cache"]))
    {
    	...
    	$button->reset_cache();
			MB()->add_notice('', __('Cache reset', 'debbuttons'));
