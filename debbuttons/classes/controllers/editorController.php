...
{
  protected $view_template = 'debbuttons-button';
  ...
  protected function loadView()
  {
    ...
    if (isset($_GET['id']) && $_GET['id'] != '') {
      ...
      if ($button_id == 0)
      {
        $error = __("debbuttons button id is zero. Your data is not saved correctly! Please check your database.","debbuttons");
      ...
      if ($return === false)
      {
        $error = __("debbuttons could not find this button in the database. It might not be possible to save this button! Please check your database or contact support! ", "debbuttons");
        MB()->add_notice('error', $error);
      }
  ...
  {
    if (! check_admin_referer("button-edit","debbuttons_button"))
  	{
  ...
  protected function updateButton()
  {
    ...
    if (isset($_POST['current_screen']))
    {
  	...
  	if (is_int($return) && $button_id <= 0)
   	...
   	if ($button_id === 0)
   	{
   		error_log(__("debbuttons Error: Button id should never be zero","debbuttons"));
   	}
