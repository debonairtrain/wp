...
use DebButtons\ScssPhp\ScssPhp\Compiler as Compiler;
...
class maxCSSParser
{
	...
	public $anchor_class = '.debbutton'; // used for matching buttons in parse_part
	...
	public function loadDom($domObj)
	{
	  ...
	  $root = $domObj->root;
		...
		// find the full and complete statement class defining debbutton. This is needed for proper parsing on parse_part.
		$anchor_element = $root->find('.debbutton', 0);
	...
	public function compile($css)
	{
		...
		$minify = get_option("debbuttons_minify", 1);

		if ($minify == 1)
		{
				$scss->setOutputStyle(\DebButtons\ScssPhp\ScssPhp\OutputStyle::COMPRESSED);
		}
	...
	*/
	protected function parse_part($element, $el_data, $el_add = '', $screenName = false)
	{
		...
		// not using scss selectors here since there is trouble w/ the :pseudo selector, which should be put on the debbutton / a tag.
		if ($element != '')
		{	$el_add .= " ." . $element;
		...
		foreach($element_data as $pseudo => $values)
		{
			...
			if ($pseudo != 'normal')
			{
				// select the debbutton case, ending with either space or next class -dot.
				// Anchor class in default situation should be .debbutton
