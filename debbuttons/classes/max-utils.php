...
class maxUtils
{
	...
	public static function ajax_action()
	{
		...
		$message = __( sprintf("No Handler found for action %s ", $plugin_action), 'debbuttons');

		if (! wp_verify_nonce($nonce, 'maxajax') )
		{
			$message = __('Nonce not verified (' . $nonce . ')', 'debbuttons');
		}
		else
		{
			do_action('debbuttons/ajax/' . $plugin_action, $_POST);
		}
   ...
   $string = mb_url_slug($string, array("transliterate" => true));
	...
	static function legacy_get_media_query($get_option = 1)
	{
		...
		$queries = array("phone" =>  "only screen and (max-width : 480px)",
		 ...
		 $query_names = array(
						  	"phone" => __("Small phones, < 480px","debbuttons"),
						  	"phone_land" => __("Small phones (landscape) ","debbuttons"),
						  	"phone_portrait" => __("Small phones (portrait), < 320px ","debbuttons"),
						  	"medium_phone" => __("Medium-size (smart)phone (480px-768px)","debbuttons"),
						  	"ipad" => __("Ipad (all) / Large phones (768px-1024px)","debbuttons"),
						  	"ipad_land" => __("Ipad landscape","debbuttons"),
						  	"ipad_portrait" => __("Ipad portrait","debbuttons"),
						  	"desktop" => __("Desktop, > 1224px","debbuttons"),
						  	"large_desktop" => __("Large desktops","debbuttons"),
						  	"custom" => __("Custom size","debbuttons"),
						  	);

		$query_descriptions = array(
							"phone" => __("Optimized for small smartphones ( screen sizes under 480px )","debbuttons"),
							"phone_land" => __("Optimzed for small smartphones in landscape and higher ( screen sizes 321px - 480px)","debbuttons"),
							"phone_portrait" => __("Optimized for small phones ( screen size max 320px )","debbuttons"),
							"ipad" => __("Optimized for devices between 768px and 1024px","debbuttons"),
							"medium_phone" => __("Optimized for medium sizes devices between 480px and 768px","debbuttons"),
							"ipad_land" => __("Optimized for devices between 768px and 1024px in landscape","debbuttons"),
							"ipad_portrait" => __("Optimized for deviced between 768px and 1024 in portrait","debbuttons"),
							"desktop" => __("Desktop screens from 1224px","debbuttons"),
							"large_desktop" => __("Large desktop screens, from 1824px","debbuttons"),
							"custom" => __("Set your own breakpoints","debbuttons"),
							);
	...
	static function get_table_name($old = false) {
		global $wpdb;
		if ($old)
			return $wpdb->prefix . 'debbuttons_buttons';
		else
			return $wpdb->prefix . 'debbuttonsv3';
	}

	static function get_collection_table_name() {
		global $wpdb;
		return $wpdb->prefix . 'debbuttons_collections';

	}

	static function get_coltrans_table_name() {
		global $wpdb;
		return $wpdb->prefix . 'debbuttons_collections_trans';
...
//		self::removeExpiredTrans();
	...
	static function fixFAConflict()
	{
		...
		$forcefa = get_option('debbuttons_forcefa');
	...
	{
		if ( ! defined('DEBBUTTONS_BENCHMARK') || DEBBUTTONS_BENCHMARK !== true)
			return;
	...
	public static function showTime()
	{
		...
		return;
			...
			$text .= "<p><strong>" . __("DebButtons Loading Time:","debbuttons") . "</strong></p>";
			$prev_time =0;
