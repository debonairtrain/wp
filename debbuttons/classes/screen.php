...
class Screen
{
  ...
  public static function setupScreens($data)
  {
  	$responsive = isset($data['responsive']) ? $data['responsive'] : array();
		$screens['default'] = array('name' => __('Main', 'debbuttons'));
		...
		$screens['new'] = array('name' => __('Add', 'debbuttons'));
...
public function getScreenTitle()
{
	...
	if ($this->is_default())
	{
		$title =  __('Your main button for all screens. ', 'debbuttons');

	}
	elseif ($this->is_new())
	{
		 $title = __('Add a new responsive screen for mobile devices', 'debbuttons');
		 $display = __('for mobile', 'debbuttons');
	}
	elseif ($min_width && $max_width)
	{
		 $display = $min_width . __('px', 'debbuttons') . ' - ' . $max_width . __('px', 'debbuttons');
		 $title = sprintf(__('Shows at screen size from %s to %s', 'debbuttons'), $min_width . __('px', 'debbuttons'), $max_width . __('px', 'debbuttons'));
	}
	elseif (! $min_width && $max_width)
	{
		 $display = '< ' . $max_width . __('px', 'debbuttons');
		 $title = sprintf(__('Shows at screen size smaller than %s', 'debbuttons'), $max_width . __('px', 'debbuttons'));

	}
	elseif (! $max_width && $min_width)
	{
		 $display = '> ' . $min_width . __('px', 'debbuttons');
		 $title = sprintf(__('Shows at screen size bigger than %s', 'debbuttons'), $min_width . __('px', 'debbuttons'));
	}
	elseif (! $min_width && ! $max_width) // if somebody does something like this :/
	{
		 $display = '';
		 $title = sprintf(__('Set width and height to use this responsive screen', 'debbuttons'));
	}
