...
class containerBlock extends maxBlock
{
	...
	public function parse_button($domObj, $mode = 'normal')
	{
		...
		if ($data["container_enabled"] == 1)
		{
			...
			$anchor->outertext = "<span class='debbutton-" . $id . "-container mb-container'>" . $anchor->outertext . "</span>";


			if ($data["container_center_div_wrap"] == 1) // I heard you like wrapping...
			{
				$anchor->outertext = "<span class='mb-center debbutton-" . $id . "-center'>" . $anchor->outertext . "</span>";
	...
	public function admin_fields($screen)
	{
		...
		/*foreach($this->fields as $field => $options)
		{
		 ...
		 $debbuttons_container_alignments = array(
	'' => '',
	...
	'float: right' => 'float: right'
	  ...
	  if ($screen->is_responsive())
		{
			$debbuttons_container_alignments['float: none'] = 'float: none';
		}

		$icon_url = MB()->get_plugin_url() . 'images/icons/' ;

				$start_block = new maxField('block_start');
				$start_block->name = __('container', 'debbuttons');
				$start_block->label = __('Container', 'debbuttons');
				$screen->addField($start_block);


				$u_container = new maxField('switch');
				$u_container->label = __('Use Container', 'debbuttons');
				$u_container->name = $screen->getFieldID('container_enabled');
				$u_container->id = $u_container->name;
				$u_container->help = __('Creates a container around the button which pushes other content', 'debbuttons');
        ...
        $screen->addField($fspacer, 'start');
				...
				$wrap_cont->label = __('Center the container', 'debbuttons');
        ...
        $screen->addField($wrap_cont, '', 'end');
				...
				$container_width->label = __('Width', 'debbuttons');
        ...
        $screen->addField($container_width, 'start', 'end');
				...
				$cwidth_unit_px->label = __('px', 'debbuttons');
				...
				$cwidth_unit_perc->label = __('%', 'debbuttons');
				...
				$screen->addField($cwidth_unit_perc, '', array('group_end', 'end'));
			 		...
			 		$ptop->label = __('Margin', 'debbuttons');
			 		$ptop->id = $screen->getFieldID('container_margin_top');
			 		$ptop->name = $ptop->id;
 					$ptop->min = 0;
			 		$ptop->inputclass = 'tiny';
			 		$ptop->before_input = '<img src="' . $icon_url . 'p_top.png" title="' . __("Margin Top","debbuttons") . '" >';
         	...
         	$screen->addField($ptop, 'start');
			 		...
			 		$pright->before_input = '<img src="' . $icon_url . 'p_right.png" class="icon padding" title="' . __("Margin Right","debbuttons") . '" >';
          ...
          $screen->addField($pright);
			 		...
			 		$pbottom->before_input = '<img src="' . $icon_url . 'p_bottom.png" class="icon padding" title="' . __("Margin Bottom","debbuttons") . '" >';
        	...
        	$screen->addField($pbottom);
			 		...
			 		$pleft->before_input = '<img src="' . $icon_url . 'p_left.png" class="icon padding" title="' . __("Margin Left","debbuttons") . '" >';
			 		$pleft->value = maxUtils::strip_px($screen->getValue($pleft->id));
          $screen->addField($pleft, '', 'end');

					$align = new maxField('option_select');
	 				$align->label = __('Alignment','debbuttons');
	 				$align->name = $screen->getFieldID('container_alignment');
	 				$align->id = $align->name;
	 				$align->selected = $screen->getValue($align->id);
	 				$align->options = $debbuttons_container_alignments; // maxUtils::selectify($align->name, $debbuttons_container_alignments, $align->value);
					$align->help = __('Float can help to align the button and other content on the same line', 'debbuttons');
