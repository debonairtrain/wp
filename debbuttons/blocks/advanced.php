...
class advancedBlock extends maxBlock
{
	...
	public function admin_fields($screen)
	{
		$data = $this->getBlockData();
				...
				$start_block->name = __('advanced', 'debbuttons');
				$start_block->label = __('Advanced', 'debbuttons');
				$screen->addField($start_block);

				$imp = new maxField('switch');
				$imp->note = __('Adding !important to the button styles can help avoid potential conflicts with your theme styles.', 'debbuttons') ;
				$imp->id = $screen->getFieldID('important_css');
				$imp->name = $imp->id;
				$imp->value = 1;
				$imp->label = __('Use !Important', 'debbuttons');
				...
				$class->name = $class->id;
				$class->label = __("Extra classes","debbuttons");
				$class->value = $screen->getValue($class->id);
				$class->note = __("Useful for custom code or other plugins who target classes", "debbuttons");
				...
				$rel->name = $rel->id;
				$rel->label = __("Custom Rel Tag","debbuttons");
				$rel->value = $screen->getValue($rel->id);
				$rel->note = __("Useful when button is targeting lightbox and/or popup plugins that use this method", "debbuttons");
			...
			//	do_action('mb-after-advanced');
				...
				$nocss->note = __('Enabling the "Use External CSS" will stop loading any button styling. You will need to put the button style manually', 'debbuttons');
				$nocss->label = __('Use External CSS', 'debbuttons');
				...
				$nospace->content = __("Warning: This will remove all styling of the buttons!","debbuttons");
			...
			//	$nospace->output('','end');
				...
				$viewcss->label = '&nbsp;';
				$viewcss->button_label = __('View CSS', 'debbuttons');
				...
				$screen->addField($endblock);
						...
						<div id="view-css" class="maxmodal-data" >
								<h3 class="title"><?php _e("External CSS","debbuttons"); ?></h3>
							<div class="content">
								<p><?php _e('If the "Use External CSS" option is enabled for this button, copy and paste the CSS code below into your theme stylesheet.', 'debbuttons') ?></p>

							<textarea id="debbutton-css" readonly="readonly">
							<?php
								if (isset($this->data["id"]) && $this->data['id'] > 0)
								{
 								...
 								{ _e("Please save the button first","debbuttons"); }

							 ?></textarea>
							 </div>
							 <div class='controls'>
							 	<p><a class='button-primary modal_close' href='javascript:void(0);'><?php _e("Close","debbuttons"); ?></a>
							 	</p>
							 </div>
						</div>
