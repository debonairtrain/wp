...
class basicBlock extends maxBlock
{
	...
	{
		parent::__construct();
		$extra_protocols = get_option('debbuttons_protocol');
	...
	public function parse_css($css, $screens, string $mode = 'normal')
	{
		...
		$css["debbutton"]["normal"]["position"] = "relative";
		$css["debbutton"]["normal"]["text-decoration"] = "none";
//		$css["debbutton"]["normal"]["white-space"] = "nowrap";  // hinders correct rendering of oneline-multilines
		$css["debbutton"]["normal"]["display"] = "inline-block";
		$css["debbutton"]["normal"]["vertical-align"] = 'middle';
		//$css["debbutton"]["normal"]["overflow"] = "hidden"; // hinder tooltip


		// option to show border boxed buttons in preview area.
		$border_box = get_option('debbuttons_borderbox');

		if ($border_box == 1)
		{
			$css['debbutton']['normal']['box-sizing']  = 'border-box';
		}
	...
	public function check_unique_name($name)
	{
		...
		if (count($results) > 0)
		{
			$message = __('Button name already used. Using non-unique names with the shortcode can cause issues', 'debbuttons');
			$message .=  ' ' . __('Already used in : ');
			foreach($results as $id)
			{
				$url =  admin_url() . 'admin.php?page=debbuttons-controller&action=edit&id=' . $id;
	...
	public function admin_fields($screen)
	{
		...
		$icon_url = MB()->get_plugin_url() . 'images/icons/';
					...
					$start_block->name = __('basic', 'debbuttons');
					$start_block->label = __('Basics', 'debbuttons');
					...
					$field_name->label = __('Button Name', 'debbuttons');
					...
					$field_name->placeholder = __("Button Name","debbuttons");
					$field_name->help = __('<p>Internal name for button. Specify purpose or use. Additional info can be added to description.</p> <p>You can use button name in the shortcode <br> [debbutton name="button-name"] </p>
						<p class="shortcode"> Shortcode attribute : name </p>', 'debbuttons');
					...
					$field_url->label = __('URL', 'debbuttons');
				//	$field_url->note = __('The link when the button is clicked.', 'debbuttons');
					$field_url->id = $screen->getFieldID('url');
					$field_url->value = rawurldecode($screen->getValue($field_url->id) );
					$field_url->placeholder = __("http://","debbuttons");
					...
					<p class='shortcode'> Shortcode attribute : url </p>", 'debbuttons');
					...
					$url_button->button_label = __('Select Site Content', 'debbuttons');
				  ...
				  $screen->addField($fspacer, 'start');
					...
					$fwindow->label_after = __('Open in New Window', 'debbuttons');
				...
				//	$screen->addField($fspacer, 'start');
					...
					$ffollow->label_after = __('Use rel="nofollow"', 'debbuttons');
					$ffollow->value = 1;
					...
					$fdownload->label_after = __('URL is download', 'debbuttons');
					$fdownload->value = 1;
					...
					$fdownload->help = __('This will tell the browser to download the URL', 'debbuttons');
					$screen->addField($fdownload, '', 'end');

					$field_title = new maxField();
					$field_title->label = __('Button Tooltip', 'debbuttons');
					...
					<p class="shortcode">Shortcode attribute : linktitle</p>', 'debbuttons');
					$screen->addField($field_title, 'start', 'end');

					// TEXT
					$field_text = new maxField();
					$field_text->label = __('Text','debbuttons');
					...
					$fonts = MB()->getClass('admin')->loadFonts();
 					...
 					$field_font->label = __('Font','debbuttons');
				...
				//	$sizes = apply_filters('mb/editor/fontsizes', maxUtils::generate_font_sizes(10,50) );
					...
					$field_size->min = 8;
					$field_size->after_input = __('px', 'debbuttons');
					...
					// Font style checkboxes
			 		...
			 		$fweight->title = __("Bold",'debbuttons');
					...
					$screen->addField($fweight, 'group_start');
			 		...
			 		$fstyle->title = __("Italic",'debbuttons');
					...
					$screen->addField($fstyle, '', 'group_end');
			 		...
			 		$falign_left->title = __('Align left','debbuttons');
					...
					$screen->addField($falign_left, 'group_start');
			 		...
			 		$falign_center->title = __('Align center','debbuttons');
					...
					$screen->addField($falign_center);
			 		...
			 		$falign_right->title = __('Align right','debbuttons');
					...
					$screen->addField($falign_right, '',  array('group_end','end') );
			 		...
			 		$ptop->label = __('Padding', 'debbuttons');
			 		$ptop->id = $screen->getFieldID('padding_top');
			 		$ptop->name = $ptop->id;
 					$ptop->min = 0;
			 		$ptop->inputclass = 'tiny';
			 		$ptop->before_input = '<img src="' . $icon_url . 'p_top.png" title="' . __("Padding Top","debbuttons") . '" >';
					...
					$screen->addField($ptop,'start');
			 		...
			 		$pright->before_input = '<img src="' . $icon_url . 'p_right.png" class="icon padding" title="' . __("Padding Right","debbuttons") . '" >';
					...
					$screen->addField($pright);
			 		...
			 		$pbottom->before_input = '<img src="' . $icon_url . 'p_bottom.png" class="icon padding" title="' . __("Padding Bottom","debbuttons") . '" >';
					...
					$screen->addField($pbottom);
			 		...
			 		$pleft->before_input = '<img src="' . $icon_url . 'p_left.png" class="icon padding" title="' . __("Padding Left","debbuttons") . '" >';
					...
					$screen->addField($pleft,'', 'end');
 					...
 					$fcolor->label = __('Text Color','debbuttons');
					...
					$screen->addField($fcolor, 'start');
 					...
 					$fcolor_hover->label = __('Text Color Hover','debbuttons');
					...
					// Fix label for px or %
					$after_input = ($screen->getValue($screen->getFieldID('button_size_unit_width')) == 'pixel') ? __('px', 'debbuttons') : __('%','debbuttons');
					$after_input = '<span class="unit">' . $after_input . '</span>';

 					// Dimension : width

 					$field_width = new maxField('number');
 					$field_width->label = __('Button Width','debbuttons');
					...
					// Fix label for px or %
					$after_input = ($screen->getValue($screen->getFieldID('button_size_unit_height')) == 'pixel') ? __('px', 'debbuttons') : __('%','debbuttons');
					$after_input = '<span class="unit">' . $after_input . '</span>';

 					// Dimension : height
 					$field_height = new maxField('number');
 					$field_height->label = __('Button Height','debbuttons');
					...
					$field_height->help = __('Width and Height are optional. When set to 0, button size will be determined by text size plus padding', 'debbuttons');
					...
					$size_spacer->label = __('Width Unit', 'debbuttons');
					...
					$wsize_unit_px->label = __('px', 'debbuttons');
					...
					$wsize_unit_perc->label = __('%', 'debbuttons');
					...
					$sp->name = 'unit-spacer ';
					$sp->label = __("Height Unit", 'debbuttons');

					$screen->addField($sp);

					// Units for height.
					$hsize_unit_px = new maxField('radio');
					$hsize_unit_px->label = __('px', 'debbuttons');
					...
					$hsize_unit_perc->label = __('%', 'debbuttons');
					...
					$hsize_unit_perc->help = __('Using percentages makes the button size to the page element. The live preview can be unreliable', 'debbuttons');
					$screen->addField($hsize_unit_perc, '', array('group_end', 'end'));


 					// Description
 					$description_hide = get_option('debbuttons_hidedescription');
 					if ($description_hide == 1)
 						$field_desc = new maxField('hidden');
					else
 	 					$field_desc = new maxField('textarea');

					$field_desc->label = __('Description', 'debbuttons');
					...
					$field_desc->placeholder = __('Brief explanation about how and where the button is used.','debbuttons');
