...
class responsiveBlock extends maxBlock
{
	...
	public function parse_css($css, $screens, string $mode = 'normal')
	{
		 ...
		 $option_fontsize = 	get_option('debbuttons_autor_font', 80);
		 $option_buttonwidth = get_option('debbuttons_autor_width', 90);
		 ...
		 if (! Screen::hasResponsive() && $option_autoresponsive == 1 && ! isset($screens['rsauto']) )
		 {
				...
				$css['debbutton']['responsive'][$prefix] = array(
		 ...
		 foreach($screens as $screenObj)
		 {
			 ...
			 foreach($this->fields as $field_id => $notused)
			 {
				 ...
				 if (isset($data[$r_field_id]))
			 	 	$css['debbutton']['responsive'][$screen_id]['definition'][$field_id] = $data[$r_field_id];
			 }
		 }
	...
	{
		 $presets = array(
					'1080px' => array('name' => __('Modern Smartphone (1080px)', 'debbuttons'), 'minwidth' => 0, 'maxwidth' => 1080),
					'768px' => array('name' =>  __('Medium Smartphone (768px)', 'debbuttons'), 'minwidth' => 768, 'maxwidth' => 1080),
					'640px' => array('name' =>  __('Small screen (640px)', 'debbuttons'), 'minwidth' => 640, 'maxwidth' => 768),
					'480px' => array('name' => __('Old Phone (480px)', 'debbuttons'), 'minwidth' => 480, 'maxwidth' => 640),
		 );

		 if ($screen->is_new())
		 {
			  $presets = array_merge(array('none' => array('name' => __('No Preset', 'debbuttons') ) ), $presets);
		 }
	...
	//	return;
		...
		$start_block->name = __('responsive', 'debbuttons');
		$start_block->label = __('Responsive Screen Settings', 'debbuttons');
		...
		$explain->label = '&nbsp;';
		$explain->content = __("<p>Add a Responsive Screen to prepare your button for mobile devices. <br> Put the width and height of the view screen you want this design to be active at. <br> Then click the 'save changes and add screen' button.</p> ", 'debbuttons');
		$explain->is_new = true;
		...
		$rexplain->label = '&nbsp';
		$rexplain->content = sprintf(__('%s The button on this screen will show with this layout on the defined screen conditions only. %s %s You can see what fields are different from the main button by the border marking. %s When hovering over the field it will show you the value of the main button :  %s %s', 'debbuttons'), '<p>','</p>', '<p>','<br>', '<input type="text" class="responsive-changed example medium disabled" disabled value="' . __('example', 'debbuttons') . '" title="' . __('Original value: example', 'debbuttons') . '">' , '</p>');
		...
		$name->name = $name->id;
		$name->label = __('Screen Name', 'debbuttons');
		$name->is_new = true;
		...
		$presetgo->button_label = __('Apply preset', 'debbuttons');
		$presetgo->is_new = true;
		$presetgo->dataaction = 'set-preset';
		$presetgo->help = __('This will fill out the width and height fields according to the chosen preset. Select a preset and click "Apply Preset"', 'debbuttons');
		$screen->addField($presetgo, '', 'end');

		$spacer = new maxField('spacer');
		$spacer->content = __('Selecting a preset will overwrite screen settings below', 'debbuttons');
		$spacer->label = '&nbsp';
		$spacer->is_new = true;
		$spacer->name = '';
		...
		$minwidth->label = __('Minimum Width', 'debbuttons');
		$minwidth->after_input = __('px',  'debbuttons');
		$minwidth->inputclass = 'small';
		$minwidth->min = 0;
		$minwidth->value = $screen->getValue($minwidth->id);
		$minwidth->help = __('Smallest screen size this screen will show. 0 for all', 'debbuttons');
		...
		$maxwidth->label = __('Maximum Width', 'debbuttons');
		$maxwidth->inputclass = 'small';
		$maxwidth->min = -1;
		$maxwidth->value = $screen->getValue($maxwidth->id);
		$maxwidth->help = __('Biggest screen size this screen will show. 0 for no maximum', 'debbuttons');
		$maxwidth->is_new = true;
		$maxwidth->after_input = __('px',  'debbuttons');
		...
		$hide->name = $hide->id;
		$hide->label = __('Hide button in this screen', 'debbuttons');
		$hide->value = 1;
		$hide->checked = checked($screen->getValue($hide->id), 1, false);
		$hide->help = __('When active, the button will not display at all in this screen. This can be used to hide the butt on devices', 'debbuttons');
		$hide->is_new = true;
		...
		$remove->button_label = __('Remove this screen', 'debbuttons');
		$remove->value = 'BAH';
		...
		$save->name = $save->id;
		$save->button_label = __('Save changes and add new responsive screen', 'debbuttons');
	...
	{
		$start_block = new maxField('block_start');
		$start_block->name = __('upgrade', 'debbuttons');
		$start_block->label = __('Upgrade Today', 'debbuttons');
		$start_block->is_new = true;
		$screen->addField($start_block);

			$output = "<div class='upgrade-responsive'>";
			$output .= "<div class='removed-note'>" . __("Save your settings first to remove the screen", 'debbuttons') . '</div>';
			$output .= '<p><h4>' . sprintf(__('You already have %s screens. To add more responsive screens to your buttons, upgrade to DebButtons PRO.', 'debbuttons'), $screen::countScreens() ) . '</b></h4>';
			$output .= '<h4>' . __('The best button editor for WordPress includes: ') . '</h4>';
			$output .= '<ul>';
			$output .= '<li>' . __('Infinite amount of screens', 'debbuttons') . '</li>';
			$output .= '<li>' . __('Icons and Images', 'debbuttons') . '</li>';
			$output .= '<li>' . __('Effects', 'debbuttons') . '</li>';
			$output .= '<li>' . __('Google Fonts and Font-Awesome', 'debbuttons') . '</li></ul>';

			$output .= '<div class="button-row"><a class="button button-primary buynow" href="https://debonairtraining.com" target="_blank">' . __('Buy Now', 'debbuttons') . '</a></div>';
			$output .= '<div class="button-row"><a class="button features" href="' . admin_url('/admin.php?page=debbuttons-pro') . '">' . __('See all features', 'debbuttons') . '</a></div>';
