...
maxAdmin.prototype.init = function () {
		...
		$(document).on('click', ".debbutton-preview", function(e) { e.preventDefault(); e.stopPropagation(); });

 		// overview input paging
 		$('#debbuttons .input-paging').on('change', $.proxy(this.do_paging, this));
  	...
  	// range inputs
		$('#debbuttons').on('change, input', 'input[type="range"]', $.proxy(this.updateRange, this ));
    ...
    $('.output .preview-toggle').on('click', $.proxy(this.toggle_preview, this));
 		...
 		$('#debbuttons input.mb-color-field').on('focus', $.proxy(this.select_field, this));
    ...
    $(window).on('debbuttons-js-init-done', $.proxy(this.loadLivePreview, this));
...
maxAdmin.prototype.openURLDialog = function(e)
{
  ...
  if (window.ajaxurl.indexOf('ajax_debbuttons') == -1) // otherwise it will start to echo
    window.ajaxurl = window.ajaxurl + '?ajax_debbuttons=editor'; // This is to catch the Ajax Queryies for Link Dialog.
