...
$page_title = __("Button editor","debbuttons");
...
$admin->get_header(array("title" => $page_title, "title_action" => $action) );
 ?>
      ...
      <input type="hidden" name="button_is_new" value="<?php echo $this->view->button_is_new ?>" />
			<?php wp_nonce_field("button-edit","debbuttons_button") ?>
			...
			*/
			...
			<div class="mb-message shortcode">
				<?php $button_name = $button->getName();
				 ...
				 ?>
				<?php _e('To use this button, place the following shortcode anywhere you want it to appear in your site content:', 'debbuttons') ?>
				<strong>[debbutton id="<?php echo $button_id ?>"]</strong>

				<p><?php _e("Shortcode options can make using DebButtons much easier! Check all possible options:", 'debbuttons'); ?> </p>

        <?php $opened = (isset($_GET['copied'])) ? 'open' : 'closed'; ?>
				<span class='shortcode-expand <?php echo $opened ?>'><h4><?php _e("How to make your life easier","debbuttons"); ?>
				...
				<div class="expanded" <?php echo ($opened == 'open') ? 'style="display:inline-block;"' : '' ?>>
					<p class="example">
						<strong><?php _e("Add a button by using the button name","debbuttons"); ?></strong>
						&nbsp; [debbutton name="<?php echo $button_name; ?>"]
					</p>
					<p class="example">
					<strong><?php _e("Same button with different link","debbuttons");  ?></strong>
						&nbsp; [debbutton id="<?php echo $button_id ?>" url="http://yoururl"]
					</p>

					<p class="example"><strong><?php _e("Same button with diffent text","debbuttons"); ?> </strong>
						&nbsp; [debbutton id="<?php echo $button_id ?>" text="yourtext"]
					</p>
					<p class="example"><strong><?php _e("All possible shortcode options","debbuttons"); ?></strong>
						&nbsp; [debbutton id="<?php echo $button_id ?>" text="yourtext" url="http://yoururl" linktitle="tooltip" window="new" nofollow="true" extraclass="extra"]
					</p>

          <p><?php _e('You can find most of these options in the dialogs you can access in classic editor, Gutenberg and various pagebuilders so you don\'t need to memorize.', 'debbuttons'); ?> </p>

					<h4><?php _e("More tips","debbuttons"); ?></h4>
					<p><?php _e("If you use this button on a static page, on multiple pages, or upload your theme to another WordPress installation choose an unique name and use ",
						"debbuttons"); ?>  <strong>[debbutton name='my-buy-button' url='http://yoururl']</strong>.
					</p>

					<p> <?php _e("By using this syntax when you edit and save your button it will be changed everywhere it is used on your site. If you delete the button and create a new one with the same name the new button will be used on your site. Easy!","debbuttons"); ?>
				 	</p>

          <h4><?php _e('Looking off on Mobile?'); ?></h4>

          <p><?php printf(__('Create an %s Responsive Screen %s with the +Add button below. You can fully customize your look on mobile devices!', 'debbuttons'), '<b>', '</b>'); ?></p>

          <h4><?php _e('Read our documentation','debbuttons'); ?></h4>

          <p><?php _e('More on shortcodes, button sizes, mobile devices: ', 'debbuttons'); ?>
              <a href="https://debonairtraining.com/documentation/" target="_blank"><?php _e('Read our documentation', 'debbuttons'); ?></a>
          </p>
    ...
    <!-- preview -->
		<div class="output" id='live-preview-modal'>
			<div class="header"><?php _e('Preview', 'debbuttons') ?> -
          ...
          <?php
            ...
            $w_unit = ($w_unit == 'pixel') ? __('px', 'debbuttons') : '%';
            $h_unit = ($h_unit == 'pixel') ? __('px', 'debbuttons') : '%';

            if ($height == 0)
            {
               $height = __('auto', 'debbuttons');
               $h_unit = '';
            }
            if ($width == 0)
            {
                $width = __('auto', 'debbuttons');
                $w_unit = '';
            }
          ...
          ?>
				...
				<p><?php _e('The top is the normal button, the bottom one is the hover.', 'debbuttons') ?></p>
          ...
          <div class='border_wrapper'>
            <div class='preview_border_height'><span style="width: <?php echo $height . $h_unit ?>"><?php echo $height . $h_unit ?></span> </div>
             ...
             <div class='preview_border_width'><span><?php echo $width . $w_unit ?></span></div>
				...
				<div class="note"><?php _e('Change this color to see your button on a different background.', 'debbuttons') ?></div>
