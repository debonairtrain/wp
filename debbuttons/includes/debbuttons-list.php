...
<?php
$page_title = __("Overview","debbuttons");
$action = $this->getButton('add-new', array('class' => 'page-title-action add-new-h2')); // "<a class='page-title-action add-new-h2' href='" . $this->getButtonLink() . "'>" . __('Add New', 'debbuttons') . "</a>";
$this->mbadmin->get_header(array("title" => $page_title, "title_action" => $action));
 ?>
			...
			<p class="status">
			<?php
				$url = $this->getListLink(); // admin_url() . "admin.php?page=debbuttons-controller&action=list";
				$trash_url =  $this->getListLink('trash'); // $url . "&view=trash";

				if ($view->listView == 'trash')
				{
					$all_line = "<strong><a href='$url'>"  .  __('All', 'debbuttons') . "</strong></a>";
					$trash_line = __("Trash", "debbuttons");
				}
				else
				{
					$all_line = __("All","debbuttons");
					$trash_line = "<a href='$trash_url'>" . __("Trash","debbuttons") . "</strong></a>";
				}
			?>
			...
			<form method="post">
				...
				<select name="bulk-action-select" id="bulk-action-select">
					<option value=""><?php _e('Bulk Actions', 'debbuttons') ?></option>
				<?php if ($view->listView == 'all'): ?>

					<option value="trash"><?php _e('Move to Trash', 'debbuttons') ?></option>
				<?php endif;
					if ($view->listView == 'trash'): ?>
						<option value="restore"><?php _e('Restore', 'debbuttons') ?></option>
						<option value="delete"><?php _e('Delete Permanently', 'debbuttons') ?></option>
				<?php endif; ?>
				</select>
				<input type="submit" class="button" value="<?php _e('Apply', 'debbuttons') ?>" />

				<?php if ($view->listView == 'trash'): ?>
					<button type="button" class='button alignright' value='empty-trash' data-buttonaction='empty-trash' data-confirm="<?php _e('Permanently delete all buttons in trash. Are you sure?', 'debbuttons-pro') ?>"><?php _e('Empty Trash', 'debbuttons'); ?></button>
				<?php endif; ?>
...
?>
				...
				<div class="button-list preview-buttons">
					...
					<div class="heading">
						...
						<span class="col col_name manage-column column-name sortable <?php echo strtolower($link_order) ?>">
							...
							<span><?php _e('Name and Description', 'debbuttons') ?></span>
						...
						<span class='col col_shortcode'><?php _e('Shortcode', 'debbuttons') ?></span>
					</div> <!-- heading -->
					...
					?>
						...
						<span class="col col_button"><div class="shortcode-container">
								...
								<?php if($view->listView == 'all') : ?>
								<a href="<?php echo $this->getButtonLink($id); ?>"><?php _e('Edit', 'debbuttons') ?></a>
									<span class="separator">|</span>
								...
								if ($view->listView == 'trash'):
								?>
								<a href="javascript:void(0);" data-buttonaction='restore' data-buttonid="<?php echo $id ?>"><?php _e('Restore', 'debbuttons') ?></a>
								<span class="separator">|</span>
								<a href="javascript:void(0);" data-buttonaction='delete' data-buttonid="<?php echo $id ?>"><?php _e('Delete Permanently', 'debbuttons') ?></a>
								<?php endif; ?>
								</div>
						...
						<span class="col col_shortcode">									[debbutton id="<?php echo $id ?>"] <br /><strong><?php _e('or', 'debbuttons'); ?></strong><br />
									[debbutton name="<?php echo $this->button->getName() ?>"]
	...
	<div class=''>
			...
			if (count($view->published_buttons) == 0):
					include('debbuttons-welcome.php');
			endif;
			?>
	</div>
