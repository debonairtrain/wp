...
if(isset($_POST['slider-id'])) {
	if (!isset( $_POST['recover_slider_nonce'] ) || ! wp_verify_nonce( $_POST['recover_slider_nonce'], 'recover_slider' ) ) {
		...
		$cis_updated_sliders = get_option('cis_updated_slider_ids',  array());
		if(is_array($cis_updated_sliders)){
			if(count($cis_updated_sliders)) {
				if(in_array($slider_id, $cis_updated_sliders)) {
		...
		if($slider_id) {
			...
			$CIS_Slides_Settings = get_post_meta( $slider_id, 'ccis_all_photos_details', true);
			$CIS_Slides_Settings_Array = unserialize(base64_decode(get_post_meta( $slider_id, 'ccis_all_photos_details', true)));
			$CIS_Slider_Slide_Count_Array = count(unserialize(base64_decode(get_post_meta( $slider_id, 'ccis_all_photos_details', true ))));

			/* echo "<pre>";
			print_r($CIS_Slides_Settings_Array);
			echo "</pre>"; */

			if(is_array($CIS_Slides_Settings_Array)){
				if(count($CIS_Slides_Settings_Array)){

					foreach($CIS_Slides_Settings_Array as $CIS_Slide_Setting){
						// get each slide attachment id by slide URL
						$slide_url = $CIS_Slide_Setting['rpgp_image_url'];
						//echo "<br />";
						if(count($attachment_id = $wpdb->get_col($wpdb->prepare("SELECT `id` FROM `$post_table_name` WHERE `guid` LIKE '%s'", $slide_url)))) {
							...
							$slide_title = $CIS_Slide_Setting['rpgp_image_label']; // attachment title
							$slide_desc = $CIS_Slide_Setting['rpgp_image_desc'];
						...
						update_post_meta($slider_id, 'ccis_all_photos_details', $slides_id_array);
					}
					...
					// ### Part - 2: Update Slider Configuration Settings Start ###
						...
						$WCIS_Gallery_Settings_Key = "WCIS_Gallery_Settings_".$slider_id;
						$WCIS_Settings_Array = unserialize(get_post_meta( $slider_id, $WCIS_Gallery_Settings_Key, true));
						//update old settings
						update_post_meta($slider_id, $WCIS_Gallery_Settings_Key, $WCIS_Settings_Array);
					...
					// ### Part - 3: Save A Flag For This Slider Start ###
						...
						$cis_updated_sliders = array();
						$cis_updated_sliders = get_option('cis_updated_slider_ids', 0);
						if(is_array($cis_updated_sliders)){
							if(count($cis_updated_sliders)) {
								array_push($cis_updated_sliders, $slider_id);
							}
						} else {
							$cis_updated_sliders = array($slider_id);
						}
						update_option('cis_updated_slider_ids', $cis_updated_sliders);
...
<div style="border: 3px dashed #23282D; padding: 50px; margin:20px;">
	...
	<p>If your slider shortcode is <strong>[CIS id=101]</strong> then your <strong>Slider ID</strong> is <strong>101</strong>.</p>
	...
	if(is_array($cis_updated_sliders)){
		if(count($cis_updated_sliders)) {
			...
			sort($cis_updated_sliders);
			foreach($cis_updated_sliders as $id){
				echo $id;
				echo ", ";
			}
			echo '</p>';
